...
MANAGEMENT_LISTEN_ADDR=127.0.0.1

if [ -n "$ENABLE_MANAGEMENT_FOR_MONITORING" ] ; then
  MANAGEMENT_LISTEN_ADDR=0.0.0.0
  log_info "enabling management listen port for exposing metrics"
fi

exec env M2_HOME=${M2_HOME} $JBOSS_HOME/bin/standalone.sh -c standalone-openshift.xml -bmanagement ${MANAGEMENT_LISTEN_ADDR} ${JAVA_PROXY_OPTIONS} ${JBOSS_HA_ARGS} ${JBOSS_MESSAGING_ARGS} "${D_ARR[@]}"
